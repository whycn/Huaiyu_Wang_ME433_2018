package com.example.huaiyuwang.afinal;
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        ...
        if (c != null) {
            ...
            for (int i = 0; i < bmp.getWidth(); i++) {
                 if ( ((red(pixels[i]) - (green(pixels[i])+blue(pixels[i]))/2) > 125-thresh) && ((red(pixels[i]) - (green(pixels[i])+blue(pixels[i]))/2) < 125+thresh) && (red(pixels[i]) > T)) {
